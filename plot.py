...
def plot_num_frequent_itemsets(dataset_names, num_frequent_itemsets):
    plt.figure(figsize=(10, 6))
    plt.bar(dataset_names, num_frequent_itemsets, color='lightgreen')
    plt.xlabel('Dataset')
    plt.ylabel('Number of Frequent Itemsets')
    plt.title('Number of Frequent Itemsets Found by NegFIN Algorithm')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.show()
...
def plot_itemsets_vs_support(min_supports, num_frequent_itemsets, dataset_name):
    plt.figure(figsize=(10, 6))
    plt.plot(min_supports, num_frequent_itemsets, marker='o', color='blue')
    plt.xlabel('Minimum Support')
    plt.ylabel('Number of Frequent Itemsets')
    plt.title(f'Frequent Itemsets vs. Minimum Support for {dataset_name}')
    plt.grid(True)
    plt.tight_layout()
    plt.show()

def combined_plot(dataset_names, execution_times, num_frequent_itemsets):
    fig, ax1 = plt.subplots(figsize=(10, 6))

    color = 'tab:blue'
    ax1.set_xlabel('Dataset')
    ax1.set_ylabel('Execution Time (ms)', color=color)
    ax1.bar(dataset_names, execution_times, color=color, alpha=0.6)
    ax1.tick_params(axis='y', labelcolor=color)

    ax2 = ax1.twinx()
    color = 'tab:green'
    ax2.set_ylabel('Number of Frequent Itemsets', color=color)
    ax2.plot(dataset_names, num_frequent_itemsets, color=color, marker='o')
    ax2.tick_params(axis='y', labelcolor=color)

    plt.title('Execution Time and Number of Frequent Itemsets for NegFIN Algorithm')
    fig.tight_layout()
    plt.show()
